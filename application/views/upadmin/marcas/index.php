        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <h2 class="page-header text-primary">Marcas</h2>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            ...
            <div class="row">
            	<div class="col-lg-12 col-md-12 col-sm-12">
            		<button class="btn btn-default pull-right" onclick="window.location.href='<?=base_url()?>index.php/marcasAdmin/criar'"><i class="fa fa-plus-circle" aria-hidden="true"></i>
 Adicionar</button>
            		<table class="table table-striped">
					    ...
					    <tbody class="dados-item">
					      <?php foreach ($marcas as $item) { ?>
						      <tr data-codigo="<?=$item['COD_MARCA']?>">
						        <td class="hidden"><?=$item['COD_MARCA']?></td>
						        <td><a href="<?=base_url()?>/index.php/marcasAdmin/editar?id=<?=$item['COD_MARCA']?>" class="text-primary"><?=$item['NOME_MARCA']?></a></td>
						        <td>
						        <?php 
						        	echo (bool)$item['TIP_ATIVO'] == true ? '<span class="label label-success">Ativo</span>' : '<span class="label label-danger">Inativo</span>';
						        ?>
						        </td>
						        <td class="text-right"><i onclick="window.location.href='<?=base_url()?>/index.php/marcasAdmin/editar?id=<?=$item['COD_MARCA']?>'" style="cursor:pointer;"  class="fa fa-pencil text-primary btn-editar" aria-hidden="true"></i> / <i style="cursor:pointer;" class="fa fa-trash-o text-danger btn-remover" aria-hidden="true"></i></td>
						      </tr>
					      <?php	} ?>
					    </tbody>
					</table>
